...
def main(args):
    ...
    with open('table.txt','w') as w:
        w.write('{bench}  &  {qubit}  &  {gate}  &  {ideal}  &  {cost_TOQM}  &  {time_TOQM}  &  {cost_greedy}  &  {time_greedy}  &  {C_greedy}  &  {T_greedy}  &  {cost_search}  &  {time_search}  &  {C_search}  &  {T_search}  \\\\\n'.format(
        ...
        for benchmark in tabless.keys():
            ...
            w.write('{bench}  &  {qubit}  &  {gate}  &  {ideal}  &  {cost_TOQM}  &  {time_TOQM}  &  {cost_greedy}  &  {time_greedy}  &  {C_greedy}  &  {T_greedy}  &  {cost_search}  &  {time_search}  &  {C_search}  &  {T_search}  \\\\\n'.format(
                ...
                cost_TOQM = "{:,}".format(table[benchmark]["cost_TOQM"]).rjust(7, ' '),
                time_TOQM = "{:,.2f}".format(table[benchmark]["time_TOQM"]).rjust(7, ' '),
                cost_greedy = "{:,}".format(table[benchmark]["cost_greedy"]).rjust(7, ' '),
                time_greedy = "{:,.2f}".format(table[benchmark]["time_greedy"]).rjust(7, ' '),
                C_greedy = "{:,.2f}".format((table[benchmark]["cost_TOQM"]-table[benchmark]["cost_greedy"])*100/table[benchmark]["cost_greedy"]).rjust(7, ' '),
                T_greedy = "{:,.1f}".format((table[benchmark]["time_TOQM"]/table[benchmark]["time_greedy"])).rjust(7, ' '),
                cost_search = "{:,}".format(table[benchmark]["cost_search"]).rjust(7, ' '),
                time_search = "{:,.2f}".format(table[benchmark]["time_search"]).rjust(7, ' '),
                C_search = "{:,.2f}".format((table[benchmark]["cost_TOQM"]-table[benchmark]["cost_search"])*100/table[benchmark]["cost_search"]).rjust(7, ' '),
                T_search = "{:,.1f}".format((table[benchmark]["time_TOQM"]/table[benchmark]["time_search"])).rjust(7, ' '),
            ))
def parse_args() -> Namespace:
    parser = ArgumentParser()
    parser.add_argument('--transform2HHMMSS', default=False, action=BooleanOptionalAction)
