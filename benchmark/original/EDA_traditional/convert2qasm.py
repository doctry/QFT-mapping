...
def getQasmBit(
    bits: List[str], qcBit2qasmBit: Dict[str, int], qasmBit: int
) -> List[int]:
    result = []
    for bit in bits:
...
def main(args):
    ...
    with open(
        "{root}/{file}.qasm".format(root=args.output_root, file=outfile), "w"
    ) as qasmf:
...
def parse_args() -> Namespace:
    ...
    parser.add_argument("--qc", type=str, help="Input qc file", required=True)
    parser.add_argument(
