...
for i in range(len(qc)):
    ...
    if gate[1] == "CXGate object":
        qubit = parse("[Qubit(QuantumRegister(%d, 'q'), {}), Qubit(QuantumRegister(%d, 'q'), {})]" %(qubit_number,qubit_number)  ,str(qc[i][1]))
        ...
        d = {'Operator': 'Cnot', 'Qubits': [int(qubit[0]),int(qubit[1])],'duration':[start_time,start_time+double_gate_time]}
    else:
        start_time = depth[int(qubit[0])]
        qubit = parse("[Qubit(QuantumRegister(%d, 'q'),{})]" %qubit_number ,str(qc[i][1]))
        depth[int(qubit[0])]+=single_gate_time
        d = {'Operator': str(gate[0]), 'Qubits': [int(qubit[0])],'duration':[start_time,start_time+single_gate_time]}
