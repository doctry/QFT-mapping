...
const unsigned device::Qubit::get_id() const { return _id; }

const unsigned device::Qubit::get_avail_time() const { return _occupied_until; }
...
const unsigned device::Qubit::get_topo_qubit() const { return _topo_qubit; }
...
const unsigned device::Qubit::get_cost() const { return _cost; }

const bool device::Qubit::is_marked() const { return _marked; }

const bool device::Qubit::is_taken() const { return _taken; }
const bool device::Qubit::get_swtch() const { return _swtch; }

const unsigned device::Qubit::get_pred() const { return _pred; }

const unsigned device::Qubit::get_swap_time() const { return _swap_time; }
...
device::Device::route(unsigned source, unsigned target) {
...
std::vector<Operation> device::Device::traceback(Operator op, device::Qubit &q0,
                                                 device::Qubit &q1,
                                                 device::Qubit &t0,
                                                 device::Qubit &t1) {
    ...
    if (op == Operator::R) {
        Operation R_gate(Operator::R, std::make_tuple(q0.get_id(), q1.get_id()),
                         std::make_tuple(op_time, op_time + _R_CYCLE));
        ops.push_back(R_gate);
    } else if (op == Operator::CX) {
        Operation CX_gate(Operator::CX,
                          std::make_tuple(q0.get_id(), q1.get_id()),
...
std::tuple<std::vector<unsigned>, std::vector<unsigned>>
device::Device::trace(device::Qubit &q0, device::Qubit &q1, device::Qubit &t0,
                      device::Qubit &t1) {
    assert(t0.get_id() == t0.get_pred());
    assert(t1.get_id() == t1.get_pred());
...
std::vector<Operation> device::Device::compile_route(
    const std::tuple<std::vector<unsigned>, std::vector<unsigned>> &routes) {
    ...
    Operation R_gate(Operator::R, std::make_tuple(q0.get_id(), q1.get_id()),
                     std::make_tuple(R_time, R_time + _R_CYCLE));
    ops.push_back(R_gate);
    ...
    for (unsigned i = 1; i < route_0.size(); ++i) // trace 0
    {
        ...
        Operation swap_gate(
            Operator::Swap, std::make_tuple(parent, child),
            std::make_tuple(swap_time, swap_time + _SWAP_CYCLE));
        ops.push_back(swap_gate);
    }
...
std::vector<Operation> &device::Device::get_operations() { return _ops; }