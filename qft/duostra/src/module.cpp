...
void Gate::finished(unsigned prev){
    for(size_t i=0;i<_prevs.size();i++){
        if (_prevs[i].first==prev)  _prevs[i].second = true;
    }
}

void Topology::update_avail_gates(unsigned executed){
    ...
    for (unsigned i = 0; i < nexts.size(); i++){
        ...
        if (_gates[n].is_avail())   _avail_gates.push_back(n);
    }
}

void Topology::parse(fstream& qasmFile){
    
    string str;
    for(int i=0; i<6 ;i++) qasmFile >> str;
    ...
    while(qasmFile >> str){
        ...
        if (type != "cx")   qasmFile >> str;
            ...
            Gate tempGate(cnotId,temp);
            tempGate.set_prev(_lastGate[q1],_lastGate[q2]);
            
            if (_lastGate[q1]!=unsigned(-1))    _gates[_lastGate[q1]].add_next(cnotId);
            if (_lastGate[q1]!=_lastGate[q2]){
                if (_lastGate[q2]!=unsigned(-1))    _gates[_lastGate[q2]].add_next(cnotId);
            }
    ...
    for(size_t i=0; i<_gates.size();i++){
        if (_gates[i].is_avail())   _avail_gates.push_back(i);
    }
    # ifdef DEBUG
...
void Topology::print_gates_with_next(){
...
void Topology::print_gates_with_prev(){
